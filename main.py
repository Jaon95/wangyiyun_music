...
class EncryptText:
    ...
    def AESEncrypt(self, clear_text, key):
        ...
        """
        # 数据填充
        clear_text = pad(data_to_pad=clear_text.encode(),
                         block_size=AES.block_size)
        key = key.encode()
        # iv代表初始向量，不同的iv加密后的字符串不同，目的和key类似
        iv = self.iv.encode()
        # 使用iv可key创建一个新的加密器，加密模式为CBC
...
class WangYiYunMusic(object):
    ...
    def parse_text(self, text):
        ...
        print('{0:{5}<5}{1:{5}<20}{2:{5}<10}{3:{5}<10}{4:{5}<20}'.format(
            '序号', '歌名', '歌手', '时长(s)', '专辑', chr(12288)))
        print('{:-^84}'.format('-'))
        for id_info in ids_list:
            ...
            print('{0:{5}<5}{1:{5}<20}{2:{5}<10}{3:{5}<10}{4:{5}<20}'.format(
                count, song_name, singer, time, album_name, chr(12288)))
            count += 1
            if count == 8:
                # 为了测试方便, 这里只显示了9条数据
                break
...
def wyy_main():
    ...
    while True:
        ...
        song_text = wyy.get_html(
            song_url, method='POST', from_data=song_from_data)
